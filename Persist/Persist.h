...
namespace NoSqlDatabase {
	...
	inline Persist& Persist::operator=(const Persist& persist) {
		if (this != &persist) {
	...
	inline void Persist::save(Xml xml) {
		std::fstream out;
	...
	using namespace XmlProcessing;
	inline Persist::Xml Persist::restore() {
	...
	using namespace XmlProcessing;
	inline Persist::Xml Persist::xmlToStr(const Path& inPath) {
		Xml xml;
		try
		{
